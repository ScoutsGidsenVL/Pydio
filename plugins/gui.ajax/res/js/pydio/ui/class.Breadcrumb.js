...
Class.create("Breadcrumb", AjxpPane, {
        ...
        this.observerFunc = function(event){
            ...
            parts.each(function(pair){
                if(this.options["use_ul"]){
                    ...
                    clickPath += (pair.value != pos == 0 || !this.options['hide_home_icon'] ? chevron : "") + "<span class='ajxp-goto' data-goTo='"+pair.key+"'>"+pair.value+"</span>";
                }
                pos ++;
            }.bind(this));
            if(this.options['use_ul']){
  ...
  resizeUls: function(){
    // Disabled, because it didn't work properly.
  },
